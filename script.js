...
function loadAndDisplayRouteWithTabuSeacrh() {
  fetch("./data/tabu_search.json") // Ganti dengan path ke file JSON Anda
    ...
    .then((data) => {
      ...
      document.getElementById(
        "result"
      ).innerText = `Rute Wisata: ${data.best_solution}\n Perkiraan Waktu Tempuh: ${data.best_cost} menit`;

      displayRouteWithTabuSearch(data.best_solution);
    })
    .catch((error) => {
...
function displayRouteWithTabuSearch(route) {
  const locations = [
...
function loadAndDisplayRouteWithPSO() {
  fetch("./data/pso.json") // Ganti dengan path ke file JSON Anda
    .then((response) => {
      if (!response.ok) {
        throw new Error("Network response was not ok " + response.statusText);
      }
    ...
    .then((data) => {
      ...
      displayRouteWithPSO(data.best_solution);
    })
    .catch((error) => {
      console.error("Fetch error:", error);
      document.getElementById("result").innerText =
